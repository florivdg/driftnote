...
**Local auth bypass.** Set `DRIFTNOTE_AUTH_BYPASS=1` to skip the passkey ceremony and impersonate an existing user — handy for exercising the prod build (`bun run build && bun run start`) where you can't register a passkey. `attachSession` resolves a real user row (the FKs on `ideas`/`tags` require one) instead of calling Better Auth; `DRIFTNOTE_AUTH_BYPASS_EMAIL` picks which user, else the first row. **Two conditions must both hold**: the flag is set _and_ `BETTER_AUTH_URL` is localhost. A real deployment sets `BETTER_AUTH_URL` to its public domain (the passkey `rpID` derives from it), so the flag leaking into prod env can't disable auth there — and without the flag locally the normal `/login` redirect still fires. See `.env.example`.
